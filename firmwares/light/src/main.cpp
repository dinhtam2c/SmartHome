...
bool isValidRgbCommand(JsonVariantConst commandValue) {
    ...
    int red = value["red"].as<int>();
    int green = value["green"].as<int>();
    int blue = value["blue"].as<int>();
...
bool parseHexColor(const char* value, int& red, int& green, int& blue) {
    if (value == nullptr) {
        return false;
    }
    ...
    for (int index = 0; index < 6; index++) {
        digits[index] = hexDigitValue(hex[index]);
        if (digits[index] < 0) {
            return false;
        }
    }
...
bool readFlashEffectCommandValue(
    ...
    const char* color = commandValue["color"].as<const char*>();
    if (color != nullptr && !parseHexColor(color, red, green, blue)) {
        return false;
    }
...
CapabilityDefinition buildPowerCapability() {
    ...
    capability.handleCommand = [](const std::string& operation, JsonVariantConst commandValue,
        std::string& outStateJson, std::string& outError) {
            ...
            outStateJson = boolStateJson(lightPower);
            return true;
        };
...
CapabilityDefinition buildBrightnessCapability() {
    ...
    capability.handleCommand = [](const std::string& operation, JsonVariantConst commandValue,
        std::string& outStateJson, std::string& outError) {
            ...
            outStateJson = brightnessStateJson();
            return true;
        };
...
CapabilityDefinition buildRgbCapability() {
    ...
    capability.handleCommand = [](const std::string& operation, JsonVariantConst commandValue,
        std::string& outStateJson, std::string& outError) {
            ...
            if (!isValidRgbCommand(commandValue)) {
                outError = "invalid_value";
                return true;
            }

            setRgbColor(
                commandValue["value"]["red"].as<int>(),
                commandValue["value"]["green"].as<int>(),
                commandValue["value"]["blue"].as<int>());
            outStateJson = rgbStateJson();
            return true;
        };
