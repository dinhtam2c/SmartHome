...
std::string jsonNumberOrNull(float value, int precision) {
...
CapabilityDefinition buildTemperatureCapability() {
    ...
    capability.getStateJson = []() {
        return std::string("{\"value\":") + jsonNumberOrNull(temperature, 1) + "}";
        };
...
CapabilityDefinition buildHumidityCapability() {
    ...
    capability.supportedOperations = {};
    capability.getStateJson = []() {
        return std::string("{\"value\":") + jsonNumberOrNull(humidity, 1) + "}";
        };
...
const unsigned long DHT22_READ_INTERVAL_MS = 1000;
