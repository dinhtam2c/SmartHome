...
std::string renderProvisioningPortalPage(const ProvisioningPortalViewModel& model) {
    ...
    html +=
        ...
        ".hidden{display:none}"
        ".advanced{border:1px solid #e5e7eb;border-radius:10px;background:#f9fafb}"
        ".advanced summary{cursor:pointer;font-weight:700;padding:12px 14px;list-style:none}"
        ".advanced summary::-webkit-details-marker{display:none}"
        ".advanced summary:after{content:'+';float:right}"
        ".advanced[open] summary:after{content:'-'}"
        ".advanced-fields{display:grid;gap:12px;padding:0 14px 14px}"
    ...
    html += "<h1>Device Setup</h1><p>Enter Wi-Fi settings. MQTT broker settings are under Advanced.</p>";
    ...
    html += "<details class='advanced'><summary>Advanced</summary><div class='advanced-fields'>";
    html += "<label>MQTT Broker Host<input name='server' value='" + model.server.address + "'></label>";
    html += "<label>MQTT Broker Port<input name='port' type='number' min='1' max='65535' value='" + std::to_string(serverPortToDisplay) + "'></label>";
    html += "</div></details>";
    ...
    html +=
        ...
        "return fetch('/wifi-networks',{cache:'no-store'})"
        ".then(function(r){if(!r.ok){throw new Error('wifi');}return r.json();})"
        ".then(function(payload){renderNetworks(payload.results||[]);return payload;})"
        ".catch(function(){return null;});"
        "}"
        "function pollNetworksUntilDone(deadline){"
        "refreshNetworks().then(function(payload){"
        "if(payload&&payload.scanInProgress&&Date.now()<deadline){setTimeout(function(){pollNetworksUntilDone(deadline);},1000);return;}"
        "setScanBusy(false);"
        "});"
        "}"
        ...
        ".then(function(r){if(!r.ok){throw new Error('scan');}return r.json();})"
        ".then(function(){setTimeout(function(){pollNetworksUntilDone(Date.now()+15000);},500);})"
        ".catch(function(){refreshNetworks();setScanBusy(false);});"
        "});"
