...
void Device::begin() {
    ...
    _provisioningManager.begin(_configuration, _wifiManager, _apSsid, _apPassword,
        _deviceName, _deviceCategory, _firmwareVersion, _endpoints, _capabilities);
...
void Device::handleCommandMessage(const std::string& payload) {
    ...
    if (endpointId.empty()) {
        Serial.println("[Device] ignoring command without endpointId");
        return;
    }

    if (capabilityId.empty()) {
        publishCommandResult(
            "", endpointId, correlationId, operation, "Failed", "", "capability_required");
        return;
    }

    if (operation.empty()) {
        publishCommandResult(
            capabilityId, endpointId, correlationId, operation, "Failed", "", "operation_required");
        return;
    }
    ...
    if (capability == nullptr) {
        publishCommandResult(
            capabilityId, endpointId, correlationId, operation, "Failed", "", "capability_not_found");
        return;
    }

    if (!capability->supportedOperations.empty() && !supportsOperation(*capability, operation)) {
        publishCommandResult(
            capabilityId, endpointId, correlationId, operation, "Failed", "", "operation_not_supported");
        return;
    }
    ...
    std::string outStateDeltaJson;
    std::string outError;
    bool handled = capability->handleCommand(
        operation, doc["value"], outStateDeltaJson, outError);
    if (!handled) {
        publishCommandResult(
            capabilityId, endpointId, correlationId, operation, "Failed", "", "operation_not_supported");
        return;
    ...
    if (!outError.empty()) {
        publishCommandResult(capabilityId, endpointId, correlationId, operation, "Failed", "", outError);
        return;
    }
...
void Device::publishCommandResult(const std::string& capabilityId,
    ...
    const std::string& stateDeltaJson,
    const std::string& error) {
    JsonDocument payload;
    payload["capabilityId"] = capabilityId.c_str();
    payload["endpointId"] = endpointId.c_str();
    payload["correlationId"] = correlationId.c_str();
    payload["operation"] = operation.c_str();
    payload["status"] = status.c_str();

    JsonArray stateChanges = payload["stateChanges"].to<JsonArray>();
    if (!stateDeltaJson.empty()) {
        JsonDocument stateDoc;
        if (deserializeJson(stateDoc, stateDeltaJson.c_str()) == DeserializationError::Ok &&
            stateDoc.is<JsonObject>()) {
            JsonObject stateChange = stateChanges.add<JsonObject>();
            stateChange["capabilityId"] = capabilityId.c_str();
            stateChange["endpointId"] = endpointId.c_str();
            stateChange["state"].set(stateDoc.as<JsonObjectConst>());
            _lastCapabilityState[capabilityId + "|" + endpointId] = stateDeltaJson;
        }
    }
