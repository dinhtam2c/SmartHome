...
namespace {
    ...
    std::string provisionClientIdFromMac(const std::string& macAddress) {
        return normalizeMacUpperWithColons(macAddress);
    }
...
void ProvisioningManager::begin(Configuration& configuration, WiFiManager& wifiManager,
    ...
    const std::string& deviceName, const std::string& deviceCategory,
    ...
    _provisionMessageMutex = xSemaphoreCreateMutex();

    Serial.println("[Provisioning] begin");
    Serial.printf("[Provisioning] deviceName=%s category=%s firmwareVersion=%s\r\n",
        _deviceName.c_str(), _deviceCategory.c_str(), _firmwareVersion.c_str());
...
std::string ProvisioningManager::renderWifiNetworksJson() {
    syncWifiScanResults();
...
void ProvisioningManager::handleSave() {
    ...
    if (ssid.empty() || serverAddress.empty() || serverPort == 0) {
...
bool ProvisioningManager::isWifiScanAllowed() const {
    // Scanning and joining an AP share the station radio. Once credentials are
    // saved, stop scans so they cannot overlap Wi-Fi/MQTT connection attempts.
    return _state == State::ConfigRequired;
}

void ProvisioningManager::syncWifiScanResults() {
    if (_wifiManager == nullptr) {
        return;
    }
...
bool ProvisioningManager::requestProvisionCode() {
    ...
    payload["category"] = _deviceCategory.c_str();
    ...
    Serial.printf("[Provisioning] publishing request topic=%s deviceName=%s category=%s macAddress=%s firmwareVersion=%s endpoints=%u\r\n",
        requestTopic.c_str(), _deviceName.c_str(), _deviceCategory.c_str(), macAddress.c_str(), _firmwareVersion.c_str(),
        static_cast<unsigned>(_endpoints.size()));
    uint16_t packetId = _provisionMqtt.publish(requestTopic, body, 1, false);
    if (packetId == 0) {
        _requestPublished = false;
        _message = "Provisioning request could not be queued. Retrying...";
        return false;
    }
...
void ProvisioningManager::ensureProvisionMqttStarted() {
    ...
    // Register before begin so every connection, including reconnects, restores
    // the subscription before a provisioning request is published.
    _provisionMqtt.subscribe(responseTopic, 1);

    _provisionMqtt.onConnected([this]() {
        Serial.println("[Provisioning] provisioning MQTT connected");
        _provisionMqttConnectedPending.store(true);
        });
    ...
    _provisionMqtt.onDisconnected([this](AsyncMqttClientDisconnectReason) {
        ...
        _provisionMqttDisconnectedPending.store(true);
        });
...
void ProvisioningManager::queueProvisionMessage(const std::string& payload) {
    if (_provisionMessageMutex == nullptr ||
        xSemaphoreTake(_provisionMessageMutex, pdMS_TO_TICKS(100)) != pdTRUE) {
        Serial.println("[Provisioning] dropped response: event queue is busy");
        return;
    }

    _pendingProvisionPayload = payload;
    _provisionMessagePending.store(true);
    xSemaphoreGive(_provisionMessageMutex);
}

void ProvisioningManager::processProvisionMqttEvents() {
    if (_provisionMqttDisconnectedPending.exchange(false)) {
        _requestPublished = false;
        if (_state == State::RequestingCode || _state == State::WaitingForApproval) {
            _state = State::RequestingCode;
            _provisionCode.clear();
    ...
    if (_provisionMqttConnectedPending.exchange(false)) {
        _requestPublished = false;
        if (_state == State::RequestingCode) {
            _lastProvisionAction = 0;
            requestProvisionCode();
    ...
    if (!_provisionMessagePending.exchange(false)) {
        return;
    }
    ...
    std::string payload = _pendingProvisionPayload;
    _pendingProvisionPayload.clear();
    xSemaphoreGive(_provisionMessageMutex);
...
void ProvisioningManager::loopTask(void* self) {
    ProvisioningManager* manager = static_cast<ProvisioningManager*>(self);
    while (manager->_portalActive) {
        manager->loop();
...
void ProvisioningManager::processWifiScan() {
    ...
    // Avoid scans once STA/MQTT is active; scans in AP+STA mode can destabilize
    // provisioning on ESP32.
    if (!isWifiScanAllowed()) {
        return;
    }
