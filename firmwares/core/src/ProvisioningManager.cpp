...
bool ProvisioningManager::isWifiScanAllowed(bool allowWhenConnected) const {
    if (_state == State::ConfigRequired) {
        return true;
    }

    if ((_state == State::WaitingForWifi || _state == State::RequestingCode || _state == State::WaitingForApproval) &&
        _wifiManager != nullptr) {
        return allowWhenConnected || !_wifiManager->isConnected();
    }
