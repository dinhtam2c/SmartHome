...
private:
    ...
    std::atomic<bool> _provisionMqttConnectedPending{ false };
    std::atomic<bool> _provisionMqttDisconnectedPending{ false };
    std::atomic<bool> _provisionMessagePending{ false };
    SemaphoreHandle_t _provisionMessageMutex = nullptr;
    std::string _pendingProvisionPayload;
    ...
    std::string renderWifiNetworksJson();
    void handleWifiScan();
    void handleSave();
    void handleNotFound();

    bool isWifiScanAllowed() const;
    void syncWifiScanResults();
    ...
    void queueProvisionMessage(const std::string& payload);
    void processProvisionMqttEvents();
