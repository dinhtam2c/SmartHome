    ...
    public async Task<HomeDetailsDto> Handle(GetHomeDetailsQuery request, CancellationToken cancellationToken)
    {
        ...
        var roomDtos = home.Rooms.Select(room =>
        {
            ...
            var temperatures = roomDevices
                .Where(d => d.IsOnline)
                .SelectMany(d => d.Endpoints.SelectMany(endpoint => endpoint.Capabilities))
                .Where(c =>
                    IsCapabilityKind(c.CapabilityId, "temperature"))
                .Select(c => CapabilityStateReader.TryReadNumber(c.State))
                .Where(v => v.HasValue)
                .Select(v => v!.Value)
                .ToList();
        ...
        var floorCount = await _context.Floors
            .AsNoTracking()
            .CountAsync(floor => floor.HomeId == home.Id, cancellationToken);
