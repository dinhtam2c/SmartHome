...
namespace Application.ActionSets.Validation;
...
internal static class ActionSetValidationHelper
{
    ...
    private static ActionSetExecutionPolicy ValidatePolicy(
        ActionSetExecutionPolicyModel? policy,
    ...
    private static List<ValidatedAction> ValidateActions(
        Guid homeId,
        ActionSetSection section,
        IReadOnlyList<ActionSetActionModel> actions,
        ...
        ICollection<string> errors)
    {
        ...
        var options = NormalizeOptionalDictionary(action.Options);
        ...
        return new SetStateActionDefinition(target, desiredState, options);
    }
        ...
        ICollection<string> errors)
    {
        ...
        try
        {
            var payload = capabilityCommandValidator.ValidateAndNormalize(
                capability,
                operation,
        ...
        if (action.State is not null || action.Options is not null)
            errors.Add($"{prefix}: invokeOperation does not accept state or options fields.");
        ...
        ICollection<string> errors)
    {
        ...
        try
        {
            capabilityStateValidator.Validate(capability, desiredState);
        }
        catch (Exception ex)
        {
        ...
        ICollection<string> errors)
    {
        ...
        var finalPayload = ActionStateHelper.MergePayloadOptions(
            mappedPayloadResult.Payload,
            options,
            out var collidingKeys);

        if (collidingKeys.Count > 0)
        {
            errors.Add($"{prefix}.options cannot override mapped state command fields: {string.Join(", ", collidingKeys)}.");
            return;
        }

        try
        {
            capabilityCommandValidator.ValidateAndNormalize(
                capability,
                strategy.Operation,
                finalPayload);
        }
        ...
        {
            errors.Add($"{prefix}.options produces invalid applyStrategy operation '{strategy.Operation}' payload: {ex.Message}");
        }
    }
