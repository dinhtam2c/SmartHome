using Application.Ports.Messages;
using Infrastructure.Message.Mqtt.Dtos;

namespace Infrastructure.Message.Mqtt;

public sealed class MqttDeviceTransport : IDeviceCommandSender, IDeviceProvisioningSender
{
    private readonly MqttService _mqttService;

    public MqttDeviceTransport(MqttService mqttService)
    {
        _mqttService = mqttService;
    }

    public async Task Send(DeviceCommandRequest command, CancellationToken cancellationToken)
    {
        var topic = MqttTopics.DeviceCommand(command.DeviceId);
        var message = DeviceCommandMessage.Create(
            command.EndpointId,
            command.CapabilityId,
            command.Operation,
            command.Value,
            command.CorrelationId);
        await _mqttService.PublishMessage(topic, message, 2, false);
    }

    public async Task SendCredentials(
        string macAddress,
        Guid deviceId,
        string accessToken,
        CancellationToken cancellationToken)
    {
        var topic = MqttTopics.DeviceProvisionResponse(macAddress);
        var message = new DeviceCredentialsResponseMessage(deviceId, accessToken);
        await _mqttService.PublishMessage(topic, message, 1, false);
    }

    public async Task SendProvisionCode(
        string macAddress,
        string provisionCode,
        CancellationToken cancellationToken)
    {
        var topic = MqttTopics.DeviceProvisionResponse(macAddress);
        var message = new DeviceProvisionResponseMessage(provisionCode);
        await _mqttService.PublishMessage(topic, message, 1, false);
    }
}
