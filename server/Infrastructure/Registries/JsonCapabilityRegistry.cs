...
{
    public string FilePath { get; set; } = "capability-registry/index.json";
}

public sealed class JsonCapabilityRegistry : ICapabilityRegistry
{
    private const string IndexFilesPropertyName = "files";
        ...
        ILogger<JsonCapabilityRegistry> logger)
    {
        ...
        ValidateSchemas(_definitions.Values, schemaEvaluator);
    ...
    private static Dictionary<string, CapabilityDefinition> LoadDefinitions(string registryPath)
    {
        var result = new Dictionary<string, CapabilityDefinition>(StringComparer.OrdinalIgnoreCase);

        using var document = ParseJsonDocument(registryPath);
        var root = document.RootElement;

        if (root.ValueKind != JsonValueKind.Object)
        {
            throw new InvalidOperationException(
                $"Capability registry root must be an index object: {registryPath}");
        }

        foreach (var capabilityPath in ReadCapabilityFilePaths(registryPath, root))
        {
            LoadDefinitionFromFile(capabilityPath, result);
        }
        ...
        IJsonSchemaPayloadEvaluator schemaEvaluator)
    {
        foreach (var definition in definitions)
        {
            ...
            foreach (var operation in definition.Operations.Values)
            {
                ValidateSchema(
                    definition.Id,
                    definition.Version,
                    $"operation '{operation.Name}'",
                    operation.CommandSchema,
                    schemaEvaluator);
            }
        }
    }
        ...
        IJsonSchemaPayloadEvaluator schemaEvaluator)
    {
        try
        {
            schemaEvaluator.EnsureSchemaIsValid(schemaJson);
        }
        catch (Exception ex)
        {
            throw new InvalidOperationException(
                $"Capability '{capabilityId}@{version}' has invalid {schemaName}: {ex.GetBaseException().Message}",
                ex);
        }
    }
    ...
    private static void LoadDefinitionFromFile(
        string filePath,
        IDictionary<string, CapabilityDefinition> definitions)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Capability registry file must contain one capability object: {filePath}");
        }

        AddDefinition(root, filePath, definitions);
    }

    private static IReadOnlyCollection<string> ReadCapabilityFilePaths(
        string indexPath,
        JsonElement index)
    {
        if (!index.TryGetProperty(IndexFilesPropertyName, out var filesNode)
            || filesNode.ValueKind != JsonValueKind.Array)
        {
            throw new InvalidOperationException(
                $"Capability registry index must contain a '{IndexFilesPropertyName}' array: {indexPath}");
        }

        var unsupportedProperties = index
            .EnumerateObject()
            .Select(property => property.Name)
            .Where(name => name != IndexFilesPropertyName)
            .ToList();

        if (unsupportedProperties.Count > 0)
        {
            throw new InvalidOperationException(
                $"Capability registry index only supports '{IndexFilesPropertyName}'. Unsupported properties: {string.Join(", ", unsupportedProperties)}");
        }
        ...
        {
            if (fileNode.ValueKind != JsonValueKind.String
                || string.IsNullOrWhiteSpace(fileNode.GetString()))
            {
                throw new InvalidOperationException(
                    $"Capability registry index file entry at position {fileIndex} must be a non-empty string: {indexPath}");
            }

            var capabilityPath = ResolveFilePath(indexDirectory, fileNode.GetString()!.Trim());
            if (!File.Exists(capabilityPath))
            {
                throw new InvalidOperationException(
                    $"Capability registry entry file was not found: {capabilityPath}");
            }

            if (!seenPaths.Add(capabilityPath))
            {
                throw new InvalidOperationException(
                    $"Capability registry index contains duplicate file: {fileNode.GetString()}");
            }

            capabilityPaths.Add(capabilityPath);
            fileIndex++;
        }

        if (capabilityPaths.Count == 0)
        {
            throw new InvalidOperationException(
                $"Capability registry index must list at least one file: {indexPath}");
        }
        ...
        IDictionary<string, CapabilityDefinition> definitions)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Each capability registry entry must be an object: {sourcePath}");
        }

        var definition = ParseDefinition(entry);
        var key = ToKey(definition.Id, definition.Version);

        if (!definitions.TryAdd(key, definition))
        {
            throw new InvalidOperationException(
                $"Capability registry has duplicate entry: {definition.Id}@{definition.Version}. Source: {sourcePath}");
        }
    }

    private static CapabilityDefinition ParseDefinition(JsonElement entry)
    {
    ...
    private static string ResolveFilePath(string basePath, string configuredPath)
    {
        if (Path.IsPathRooted(configuredPath))
            return Path.GetFullPath(configuredPath);
