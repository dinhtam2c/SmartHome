...
public class AutomationAction
{
        ...
        switch (definition)
        {
            case SetStateActionDefinition setState:
                StatePayload = JsonPayloadHelper.SerializeDictionary(
                    setState.State,
                    "Automation set-state action state",
                    requireNonEmpty: true);
                OptionsPayload = JsonPayloadHelper.SerializeDictionary(
                    setState.Options,
                    "Automation set-state action options");
                break;
            case InvokeOperationActionDefinition invoke:
                ...
                Payload = JsonPayloadHelper.SerializeDictionary(
                    invoke.Payload,
                    "Automation invoke-operation action payload");
                break;
            default:
    ...
    internal static AutomationAction FromDefinition(
        Guid ruleId,
        ActionSetSection section,
        ActionDefinition definition,
        int order)
    {
        return new AutomationAction(ruleId, section, definition, order);
    }
    ...
    {
        return JsonPayloadHelper.DeserializeDictionary(StatePayload, "Automation set-state action state");
    }
