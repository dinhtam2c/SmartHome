...
public class AutomationRule : Entity
{
    ...
    public IReadOnlyCollection<AutomationAction> Actions => _actions;
    ...
    public void ReplaceActionSet(ActionSetDefinition actionSet)
    {
        ...
        if (actionSet.IsEmpty)
            throw new InvalidOperationException("Automation rule must contain at least one action.");
    ...
    private void AddActions(IReadOnlyList<ActionDefinition> actions, ActionSetSection section)
    {
        ...
        {
            _actions.Add(AutomationAction.FromDefinition(Id, section, actions[index], index));
        }
    }
