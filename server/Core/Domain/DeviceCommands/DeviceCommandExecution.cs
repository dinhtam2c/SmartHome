...
public class DeviceCommandExecution
{
    ...
    private void EnsureTransitionAllowed(CommandLifecycleStatus nextStatus)
    {
        ...
        if (Status == CommandLifecycleStatus.Accepted
            && nextStatus is CommandLifecycleStatus.Completed
                or CommandLifecycleStatus.Failed
                or CommandLifecycleStatus.TimedOut)
        {
            return;
        }
