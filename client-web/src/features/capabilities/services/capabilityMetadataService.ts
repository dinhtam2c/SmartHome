...
export function getCapabilityPrimaryStateValue(
  ...
  const normalizedPrimaryStatePath = primaryStatePath?.trim();
  if (normalizedPrimaryStatePath) {
    const byPrimaryPath = getValueByPath(state, normalizedPrimaryStatePath);
...
export function getCapabilityVisualMetadata(
  _capabilityId: string | null | undefined,
  metadata: CapabilityRegistryMetadata | null | undefined
): CapabilityVisualMetadata | null {
