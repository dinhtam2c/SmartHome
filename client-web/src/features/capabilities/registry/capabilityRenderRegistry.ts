...
export type LightEffectFlashFieldPaths = {
  count: string;
  intervalMs: string;
  color: Record<RgbChannel, string>;
};
...
const LIGHT_EFFECT_CAPABILITY_ID = "light.effect";
...
type CapabilityOperationFieldEditorKind = "lightEffectFlash";

const CAPABILITY_OPERATION_FIELD_EDITOR_REGISTRY: Record<
  string,
  CapabilityOperationFieldEditorKind
> = {
  [`${LIGHT_EFFECT_CAPABILITY_ID}:flash`]: "lightEffectFlash",
};
...
export type CapabilityFieldEditorRenderPlan<TField extends FieldLike> =
  ...
  | {
    kind: "lightEffectFlash";
    fieldPaths: LightEffectFlashFieldPaths;
    fields: TField[];
    skippedFields: TField[];
  }
  | {
...
function getCapabilityOperationFieldEditorKind(
  capabilityId: string | null | undefined,
  ...
  if (!normalizedCapabilityId || !normalizedOperation) {
    return null;
  }

  return CAPABILITY_OPERATION_FIELD_EDITOR_REGISTRY[
    `${normalizedCapabilityId}:${normalizedOperation}`
  ] ?? null;
}
...
function getLightEffectFlashFieldPaths(
  fields: Array<{ path: string; }>
): LightEffectFlashFieldPaths | null {
  const fieldPathByNormalized = getFieldPathByNormalized(fields);
  const count = fieldPathByNormalized.get("count");
  const intervalMs = fieldPathByNormalized.get("intervalms");
  const colorEntries: Array<[RgbChannel, string] | null> = RGB_CHANNELS.map((channel) => {
    const path = fieldPathByNormalized.get(`color.${channel}`);

    return path ? [channel, path] : null;
  });

  if (!count || !intervalMs || colorEntries.some((entry) => entry === null)) {
    return null;
  }
  ...
  return {
    ...
    color: Object.fromEntries(
      colorEntries.filter((entry): entry is [RgbChannel, string] => entry !== null)
    ) as Record<RgbChannel, string>,
  };
}

function getLightEffectFlashKnownPathSet(fieldPaths: LightEffectFlashFieldPaths) {
  return new Set([
    fieldPaths.count.trim().toLowerCase(),
    fieldPaths.intervalMs.trim().toLowerCase(),
    ...Object.values(fieldPaths.color).map((path) => path.trim().toLowerCase()),
  ]);
}

function resolveLightEffectFlashFieldEditorRender<TField extends FieldLike>(
  fields: TField[]
): CapabilityFieldEditorRenderPlan<TField> | null {
  const fieldPaths = getLightEffectFlashFieldPaths(fields);

  if (!fieldPaths) {
    return null;
  }

  const knownPathSet = getLightEffectFlashKnownPathSet(fieldPaths);
  const skippedFields = fields.filter(
    (field) =>
      !knownPathSet.has(field.path.trim().toLowerCase()) ||
      !isSchemaFieldRenderable(field)
  );
...
function resolveCapabilitySpecificFieldEditorRender<TField extends FieldLike>(
  ...
  const operationFieldEditorKind = getCapabilityOperationFieldEditorKind(
    capabilityId,
    operation
  );

  if (operationFieldEditorKind === "lightEffectFlash") {
    return resolveLightEffectFlashFieldEditorRender(fields);
  }
...
function getFieldPathByNormalized(fields: Array<{ path: string; }>) {
  return new Map(
    fields.map((field) => {
