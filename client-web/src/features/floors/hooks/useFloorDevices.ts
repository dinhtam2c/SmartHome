...
function sortDevices(devices: BuilderDeviceDto[]) {
...
function patchDevice(device: BuilderDeviceDto, event: RealtimeDeltaEvent) {
...
function patchCapability(device: BuilderDeviceDto, event: RealtimeDeltaEvent) {
...
export function useFloorDevices(homeId: string | null) {
  const [devices, setDevices] = useState<BuilderDeviceDto[]>([]);
