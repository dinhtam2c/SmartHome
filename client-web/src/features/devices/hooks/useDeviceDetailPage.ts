...
import {
  CAPABILITY_LABEL_KEYS, composeValueByPath, DEVICE_LEVEL_ENDPOINT_KEY, formatRgbValue, getCapabilityBooleanLabel, getCapabilityBooleanLabels, getCapabilityRgbLabels, getRgbValue, isRgbCapabilityId, localizeCapabilityCommandPath, localizeCapabilityStatePath, toEndpointKey, } from "@/features/capabilities";
...
import { type OperationRule, buildCommandValue, formatCapabilityState } from "../services/deviceCapabilityService";
...
export function useDeviceDetailPage({
  ...
  const { device, isLoading, error, commandEventVersion, reload } =
  ...
  const getLocalizedCommandKeyLabel = useCallback(
    (key: string) =>
      localizeCapabilityCommandPath(t, key, CAPABILITY_LABEL_KEYS.device),
    [t]
  );
  ...
  const summarizePayload = useCallback(
    (payload: string | null, capabilityId?: string | null) => {
      ...
      try {
        ...
        const parsedRgbText = isRgbCapabilityId(capabilityId)
          ? formatRgbValue(
            parsed,
        ...
        if (parsedRgbText) {
          return parsedRgbText;
        }

        const parsedWithLocalizedBooleans = replaceBooleanValues(parsed, t, capabilityId);
        ...
        if (typeof localizedPayload === "boolean") {
          return getLocalizedBooleanLabel(localizedPayload, t, capabilityId);
        }
  ...
  useEffect(() => {
    if (commandEventVersion === 0 || !isCommandHistoryVisible) {
      return;
    }
