...
import type { DeviceCommandExecutionDto, DeviceCommandStatus } from "../types/deviceTypes";
...
type Props = {
  ...
  summarizePayload: (payload: string | null, capabilityId?: string | null) => string;
};
          ...
          <CellGrid>
            {visibleCommandHistory.map((command) => (
              <Cell
                key={command.id}
                id={command.id}
                title={localizeCapabilityOperation(
                  t,
                  command.operation,
                  CAPABILITY_LABEL_KEYS.device
                )}
                subtitle={
                  <>
                    <div>
                    ...
                    <details className={pageStyles.advancedDetails}>
                      ...
                      <div className={pageStyles.capabilityLine}>{t("request")}: {summarizePayload(command.requestPayload, command.capabilityId)}</div>
                      <div className={pageStyles.capabilityLine}>{t("result")}: {summarizePayload(command.resultPayload, command.capabilityId)}</div>
                    </details>
                  </>
                }
                onClick={() => void 0}
