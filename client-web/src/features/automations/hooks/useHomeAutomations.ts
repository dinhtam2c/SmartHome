...
export function useHomeAutomations({
  ...
  const [builderDevices, setBuilderDevices] = useState<BuilderDeviceDto[]>([]);
  const [builderDevicesByRoom, setBuilderDevicesByRoom] =
  ...
  const loadBuilderDevices = useCallback(async () => {
    ...
    setBuilderDevices(devices);
    setBuilderDevicesByRoom(Object.fromEntries(roomEntries));
    return devices;
  }, [homeId, rooms]);
  ...
  const openEditModal = useCallback(
    ...
    [form, homeId, loadBuilderDevices, showErrorToast]
  );
  ...
  const handleSave = useCallback(
    ...
    [builderDevices, capabilityRegistry.registryMap, form, homeId, loadRules]
  );
