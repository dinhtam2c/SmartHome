...
function attachRoomIdToAction(action: ActionDraft, devices: SelectableDeviceDto[]): ActionDraft {
...
export function useHomeDetailPageController({
  ...
  const [sceneDevices, setSceneDevices] = useState<
    SelectableDeviceDto[]
  >([]);
  const [sceneDevicesByRoom, setSceneDevicesByRoom] =
    useState<Record<string, SelectableDeviceDto[]>>({});
  const [isSceneDevicesLoading, setIsSceneDevicesLoading] = useState(false);
  const [sceneDevicesError, setSceneDevicesError] =
  ...
  const loadSceneDevices = useCallback(async (): Promise<SelectableDeviceDto[]> => {
    if (!homeId) {
      setSceneDevices([]);
      setSceneDevicesByRoom({});
      setSceneDevicesError(null);
      setIsSceneDevicesLoading(false);
      return [];
    }

    setIsSceneDevicesLoading(true);
    setSceneDevicesError(null);
    ...
    try {
      ...
      setSceneDevices(devices);
      setSceneDevicesByRoom(Object.fromEntries(roomEntries));
      return devices;
    } catch (loadError) {
      const message =
        (loadError as Error).message || "scenes.errors.loadSceneDevicesFailed";
      setSceneDevicesError(message);
      return [];
    } finally {
      setIsSceneDevicesLoading(false);
    }
  ...
  const openEditQuickActionModal = useCallback(
    ...
    [homeId, loadSceneDevices, showSceneToast]
  );
