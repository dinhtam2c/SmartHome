...
import { SceneUpsertModal } from "@/features/scenes";
import { HomeAutomationsSection } from "@/features/automations";
...
import { CreateRoomModal, EditRoomModal } from "@/features/rooms";
import { ClaimDeviceModal } from "@/features/devices";
  ...
  return (
    <div className={styles.pageStack}>
        ...
        action={
          <div className={styles.metaRow}>
            ...
            <Button
              variant="secondary"
              size="sm"
              onClick={() => navigate(`/homes/${home.id}/floors`)}
            >
              {t("detail.homePlan")}
            </Button>
      ...
      <DetailsView>
        ...
        <DetailRow label={t('detail.floorCount')}>{home.floorCount}</DetailRow>
      ...
      <HomeAutomationsSection homeId={home.id} rooms={home.rooms} />
      ...
      <SceneUpsertModal
        ...
        title={vm.sceneModalMode === "create" ? tScenes("scenes.create") : tScenes("scenes.details")}
        submitLabel={tScenes("scenes.saveShort")}
        submittingLabel={tScenes("scenes.saving")}
        name={vm.sceneName}
        description={vm.sceneDescription}
        isEnabled={vm.sceneIsEnabled}
        actionSet={vm.sceneActionSet}
        rooms={vm.sceneRooms}
        availableDevices={vm.sceneDevices}
        availableDevicesByRoom={vm.sceneDevicesByRoom}
        registryMap={vm.sceneRegistryMap}
        isSaving={vm.isSceneSaving}
        isDeleting={vm.isSceneDeleting}
        error={
          vm.sceneModalError ||
          vm.sceneDevicesError ||
          vm.sceneRegistryError
        }
        onClose={vm.closeSceneModal}
        onSubmit={vm.handleSaveQuickAction}
        onDelete={vm.handleDeleteSceneFromModal}
        deleteLabel={tScenes("scenes.deleteShort")}
        deletingLabel={tScenes("scenes.deleting")}
        onNameChange={vm.setSceneName}
        onDescriptionChange={vm.setSceneDescription}
        onEnabledChange={vm.setSceneIsEnabled}
        onActionSetChange={vm.setSceneActionSet}
      />
    </div>
  );
}
